...
<?php $__env->startSection('content'); ?>
    <div class="flex items-baseline justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-slate-600">Month-to-date · <?php echo e(\Carbon\Carbon::parse($analytics['period']['month_start'])->format('M j')); ?> – <?php echo e(\Carbon\Carbon::parse($analytics['period']['month_end'])->format('M j, Y')); ?></p>
        </div>
        <div class="flex gap-2">
            <a href="<?php echo e(route('sales.create')); ?>" class="px-3 py-2 rounded-md bg-indigo-600 text-white text-sm hover:bg-indigo-700">+ New sale</a>
            <a href="<?php echo e(route('expenses.create')); ?>" class="px-3 py-2 rounded-md border border-slate-300 bg-white text-sm hover:bg-slate-50">+ New expense</a>
        </div>
    </div>

    
    <div class="grid md:grid-cols-4 gap-4 mb-6" id="kpi-grid"
         data-revenue="<?php echo e($k['revenue']['current'] ?? 0); ?>"
         data-profit="<?php echo e($k['profit']['current'] ?? 0); ?>"
         data-expenses="<?php echo e($k['expenses']['current'] ?? 0); ?>"
         data-net="<?php echo e($k['net']['current'] ?? 0); ?>">
        <div class="bg-white rounded-lg border border-slate-200 p-5 transition-all" data-kpi-card="revenue">
            ...
            <div class="mt-2 text-2xl font-bold" data-kpi="revenue"><?php echo e($fmt($k['revenue']['current'] ?? 0)); ?></div>
            <div class="mt-1"><?php echo $chip($k['revenue']['change_pct'] ?? null); ?> <span class="text-xs text-slate-500">vs last month</span></div>
        </div>
        <div class="bg-white rounded-lg border border-slate-200 p-5 transition-all" data-kpi-card="profit">
            ...
            <div class="mt-2 text-2xl font-bold" data-kpi="profit"><?php echo e($fmt($k['profit']['current'] ?? 0)); ?></div>
            <div class="mt-1"><?php echo $chip($k['profit']['change_pct'] ?? null); ?> <span class="text-xs text-slate-500">margin <?php echo e($k['margin_pct'] ?? 0); ?>%</span></div>
        </div>
        <div class="bg-white rounded-lg border border-slate-200 p-5 transition-all" data-kpi-card="expenses">
            ...
            <div class="mt-2 text-2xl font-bold" data-kpi="expenses"><?php echo e($fmt($k['expenses']['current'] ?? 0)); ?></div>
            <div class="mt-1"><?php echo $chip($k['expenses']['change_pct'] ?? null); ?> <span class="text-xs text-slate-500">vs last month</span></div>
        </div>
        <div class="bg-white rounded-lg border border-slate-200 p-5 transition-all" data-kpi-card="net">
            ...
            <div class="mt-2 text-2xl font-bold <?php echo e(($k['net']['current'] ?? 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'); ?>" data-kpi="net"><?php echo e($fmt($k['net']['current'] ?? 0)); ?></div>
            <div class="mt-1"><?php echo $chip($k['net']['change_pct'] ?? null); ?> <span class="text-xs text-slate-500">profit – expenses</span></div>
        </div>
    </div>
    ...
    <section class="bg-white rounded-lg border border-slate-200 p-5 mb-6">
        <div class="flex items-center justify-between mb-3">
            ...
            <a href="<?php echo e(route('insights.index')); ?>" class="text-xs text-indigo-600 hover:underline">AI Insights →</a>
        </div>
    ...
    <div class="grid lg:grid-cols-3 gap-4 mb-6">
        ...
        <section class="bg-white rounded-lg border border-slate-200 p-5">
            ...
            <?php if(count($analytics['expense_breakdown']) === 0): ?>
                <p class="text-sm text-slate-500">No expenses this month yet.</p>
            <?php else: ?>
                <?php $totalExp = array_sum(array_column($analytics['expense_breakdown'], 'total')); ?>
                <div class="space-y-2">
                    <?php $__currentLoopData = $analytics['expense_breakdown']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $pct = $totalExp > 0 ? ($row['total'] / $totalExp * 100) : 0; ?>
                        <div>
                            <div class="flex justify-between text-xs mb-1">
                                <span class="text-slate-700"><span class="inline-block w-2 h-2 rounded-full mr-1" style="background:<?php echo e($row['color']); ?>"></span><?php echo e($row['category']); ?></span>
                                <span class="text-slate-500"><?php echo e($fmt($row['total'])); ?> · <?php echo e(number_format($pct, 1)); ?>%</span>
                            </div>
                            <div class="h-1.5 rounded bg-slate-100 overflow-hidden">
                                <div class="h-full rounded" style="width: <?php echo e($pct); ?>%; background: <?php echo e($row['color']); ?>"></div>
                            </div>
                        </div>
    ...
    <div class="grid lg:grid-cols-2 gap-4 mb-6">
        <section class="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <header class="px-5 py-3 border-b border-slate-200 flex items-center justify-between">
                ...
                <a class="text-xs text-indigo-600 hover:underline" href="<?php echo e(route('customers.index')); ?>">View all →</a>
            </header>
            <table class="w-full text-sm">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $analytics['top_customers']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="border-t border-slate-100">
                            <td class="px-5 py-2">
                                <div class="font-medium"><?php echo e($c['name']); ?></div>
                                <?php if($c['company']): ?><div class="text-xs text-slate-500"><?php echo e($c['company']); ?></div><?php endif; ?>
                            </td>
                            <td class="px-5 py-2 text-right text-slate-700"><?php echo e($fmt($c['revenue'])); ?></td>
                            <td class="px-5 py-2 text-right <?php echo e($c['profit'] >= 0 ? 'text-emerald-600' : 'text-rose-600'); ?>"><?php echo e($fmt($c['profit'])); ?></td>
                        </tr>
        ...
        <section class="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <header class="px-5 py-3 border-b border-slate-200 flex items-center justify-between">
                ...
                <a class="text-xs text-indigo-600 hover:underline" href="<?php echo e(route('products.index')); ?>">View all →</a>
            </header>
            <table class="w-full text-sm">
                <thead class="bg-slate-50 text-slate-500 text-xs uppercase">
                    <tr>
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $analytics['top_products']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="border-t border-slate-100">
                            <td class="px-5 py-2"><?php echo e($p['name']); ?></td>
                            <td class="px-5 py-2 text-right text-slate-700"><?php echo e($p['qty_sold']); ?></td>
                            <td class="px-5 py-2 text-right text-slate-700"><?php echo e($fmt($p['revenue'])); ?></td>
                            <td class="px-5 py-2 text-right <?php echo e($p['profit'] >= 0 ? 'text-emerald-600' : 'text-rose-600'); ?>"><?php echo e($fmt($p['profit'])); ?></td>
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
    ...
    <div class="grid lg:grid-cols-3 gap-4">
        <section class="lg:col-span-2 bg-white rounded-lg border border-slate-200 overflow-hidden">
            <header class="px-5 py-3 border-b border-slate-200 flex items-center justify-between">
                ...
                <a href="<?php echo e(route('activity.index')); ?>" class="text-xs text-indigo-600 hover:underline">Full audit log →</a>
            </header>
            <ul id="activity-feed" class="divide-y divide-slate-100 text-sm">
                <?php $__empty_1 = true; $__currentLoopData = $analytics['recent_activity']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php
                    ...
                    <li class="px-5 py-3 flex items-center gap-3">
                        <span class="inline-flex items-center justify-center w-7 h-7 rounded-full shrink-0 <?php echo e($iconCls); ?> font-semibold text-xs"><?php echo e($icon); ?></span>
                        <div class="flex-1 min-w-0">
                            <div class="text-sm">
                                <span class="font-medium"><?php echo e($a['causer']); ?></span>
                                <span class="text-slate-600"><?php echo e($a['description']); ?></span>
                            </div>
                            <div class="text-xs text-slate-500"><?php echo e(\Carbon\Carbon::parse($a['at'])->diffForHumans()); ?></div>
                        </div>
                    </li>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
        ...
        <section class="bg-white rounded-lg border border-slate-200 overflow-hidden">
            <header class="px-5 py-3 border-b border-slate-200 flex items-center justify-between">
                ...
                <span class="text-xs text-slate-500"><?php echo e($users->count()); ?></span>
            </header>
            <ul class="divide-y divide-slate-100 text-sm">
                <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $u): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li class="px-5 py-3 flex items-center justify-between">
                        <div>
                            <div class="font-medium"><?php echo e($u->name); ?> <?php if($u->id === auth()->id()): ?><span class="text-xs text-slate-400">(you)</span><?php endif; ?></div>
                        ...
                        <div>
                            <?php $__currentLoopData = $u->roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <span class="inline-block text-xs px-2 py-0.5 rounded bg-indigo-50 text-indigo-700 border border-indigo-200"><?php echo e($role->name); ?></span>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                    </li>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            ...
            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('invite-users')): ?>
                <div class="border-t border-slate-200 p-4 bg-slate-50">
                    ...
                    <form method="POST" action="<?php echo e(route('invitations.store')); ?>" class="space-y-2">
                        <?php echo csrf_field(); ?>
                ...
                <tbody>
                    <?php $__currentLoopData = $pendingInvitations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invitation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="border-t border-slate-100">
                            ...
                            <td class="px-5 py-3"><a href="<?php echo e($invitation->acceptUrl()); ?>" class="text-xs text-indigo-600 hover:underline break-all"><?php echo e(\Str::limit($invitation->acceptUrl(), 50)); ?></a></td>
                        </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </tbody>
            </table>
    ...
    <script>
        const trend = <?php echo json_encode($analytics['profit_trend'], 15, 512) ?>;
...
<?php echo $__env->make('layouts.tenant', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\profitlens\resources\views/tenant/dashboard.blade.php ENDPATH**/ ?>