...
@section('content')
    ...
    <div class="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
        @foreach ($plans as $slug => $plan)
            @php $highlight = $plan['highlight'] ?? false; @endphp

            <div class="bg-white rounded-lg p-6 border-2 {{ $highlight ? 'border-indigo-500 shadow-lg' : 'border-slate-200' }} relative flex flex-col">
                @if ($highlight)
                    <span class="absolute -top-3 right-4 px-2 py-0.5 text-xs font-medium bg-indigo-600 text-white rounded">Most popular</span>
                @endif

                <div>
                    <div class="text-sm font-semibold uppercase tracking-wide {{ $highlight ? 'text-indigo-600' : 'text-slate-500' }}">
                        {{ $plan['name'] }}
                    </div>
                    ...
                    <div class="mt-2 flex items-baseline gap-1">
                        ...
                        @else
                            <span class="text-4xl font-bold">{{ $fmt($plan['price_cents']) }}</span>
                    ...
                    <p class="mt-3 text-sm text-slate-600">
                        @switch($slug)
                            @case('free') Get started for free. Perfect for solo founders. @break
                            @case('pro') For growing teams that need real headroom. @break
                            @case('enterprise') For organizations that need scale + compliance. @break
                        @endswitch
                    </p>
                    ...
                    {{-- Hard limits visible up front (no surprises) --}}
                    <div class="mt-5 pt-5 border-t border-slate-100 text-xs text-slate-500 space-y-1">
                        ...
                        <div class="flex justify-between"><span>Team members</span><span class="font-mono">{{ $plan['limits']['users'] === PHP_INT_MAX ? '∞' : $plan['limits']['users'] }}</span></div>
                        <div class="flex justify-between"><span>Sales / month</span><span class="font-mono">{{ $plan['limits']['sales_per_month'] === PHP_INT_MAX ? '∞' : number_format($plan['limits']['sales_per_month']) }}</span></div>
                        <div class="flex justify-between"><span>Products</span><span class="font-mono">{{ $plan['limits']['products'] === PHP_INT_MAX ? '∞' : number_format($plan['limits']['products']) }}</span></div>
                        <div class="flex justify-between"><span>Receipt storage</span><span class="font-mono">{{ $plan['limits']['storage_mb'] >= 51200 ? '50 GB' : ($plan['limits']['storage_mb'] >= 1024 ? round($plan['limits']['storage_mb']/1024) . ' GB' : $plan['limits']['storage_mb'] . ' MB') }}</span></div>
                    </div>
                </div>
                ...
                <div class="mt-6 pt-6">
                    ...
                    @elseif ($slug === 'free')
                        <a href="{{ route('signup') }}?plan=free"
                           class="block w-full text-center px-4 py-2.5 rounded-md border border-slate-300 hover:bg-slate-50 text-sm font-medium">
                            Start free
                        </a>
                    @else
                        <a href="{{ route('signup') }}?plan={{ $slug }}"
    ...
    <div id="login-helper" class="hidden mt-6 max-w-md mx-auto bg-white rounded-lg border border-slate-200 p-5">
        ...
        <form onsubmit="event.preventDefault(); window.location='http://'+document.getElementById('sub').value+'.{{ config('app.tenant_domain') }}:'+window.location.port+'/login'">
    ...
    {{-- Status banner: shows whether Stripe is wired or in dev mode --}}
