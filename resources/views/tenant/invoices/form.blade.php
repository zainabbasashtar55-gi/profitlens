...
@section('title', $invoice->exists ? 'Edit invoice ' . $invoice->invoice_number : 'New invoice')

@section('content')
    <div class="max-w-4xl">
        <h1 class="text-2xl font-bold mb-1">{{ $invoice->exists ? 'Edit ' . $invoice->invoice_number : 'New invoice' }}</h1>
        ...
        <form method="POST"
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">Customers without an email can't be sent invoices. <a href="{{ route('customers.create') }}" class="text-indigo-600 hover:underline">+ Add customer</a></p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Currency</label>
                    <input name="currency" value="{{ old('currency', $invoice->currency ?? 'USD') }}" maxlength="3" class="w-full rounded-md border border-slate-300 px-3 py-2 uppercase font-mono">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Issue date *</label>
                    <input type="date" name="issue_date" required value="{{ old('issue_date', $invoice->issue_date instanceof \Carbon\Carbon ? $invoice->issue_date->toDateString() : $invoice->issue_date) }}" class="w-full rounded-md border border-slate-300 px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Due date *</label>
                    <input type="date" name="due_date" required value="{{ old('due_date', $invoice->due_date instanceof \Carbon\Carbon ? $invoice->due_date->toDateString() : $invoice->due_date) }}" class="w-full rounded-md border border-slate-300 px-3 py-2">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Payment terms</label>
                    <input name="payment_terms" value="{{ old('payment_terms', $invoice->payment_terms) }}" placeholder="e.g. Net 14" class="w-full rounded-md border border-slate-300 px-3 py-2">
                </div>
            </div>
            ...
            <div>
                    ...
                    <tbody id="items-body">
                        ...
                        @foreach ($rows as $i => $row)
                            <tr class="border-t border-slate-100" data-row>
                                <td class="py-2 pr-2">
                                    <input name="items[{{ $i }}][description]" required value="{{ $row['description'] ?? '' }}"
                                           class="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
                                           list="product-list" data-field="description">
                                    <input type="hidden" name="items[{{ $i }}][product_id]" value="{{ $row['product_id'] ?? '' }}" data-field="product_id">
                                    <input type="hidden" name="items[{{ $i }}][unit_cost]"  value="{{ $row['unit_cost'] ?? 0 }}"  data-field="unit_cost">
                                </td>
                ...
                <datalist id="product-list">
                    @foreach ($products as $p)
                        <option data-id="{{ $p->id }}" data-price="{{ (float) $p->sell_price }}" data-cost="{{ (float) $p->cost_price }}" value="{{ $p->name }}"></option>
                    @endforeach
                </datalist>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" rows="3" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="Thanks for your business!">{{ old('notes', $invoice->notes) }}</textarea>
            </div>

            <div class="flex gap-2 pt-2">
                <button class="px-4 py-2 rounded-md bg-indigo-600 text-white text-sm hover:bg-indigo-700">
                    {{ $invoice->exists ? 'Save changes' : 'Save as draft' }}
                </button>
                <a href="{{ $invoice->exists ? route('invoices.show', $invoice) : route('invoices.index') }}" class="px-4 py-2 rounded-md border border-slate-300 bg-white text-sm">Cancel</a>
            </div>
        </form>
    </div>
