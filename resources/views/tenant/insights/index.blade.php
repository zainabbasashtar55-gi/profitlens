...
@section('content')
            ...
            <div class="border-t border-slate-200 p-3">
                <div class="flex flex-wrap gap-1.5 mb-2" id="chat-suggestions">
                    @foreach ([
                        ...
                        <button type="button" class="suggestion text-xs px-2 py-1 rounded-full border border-slate-200 text-slate-600 hover:bg-slate-50">{{ $s }}</button>
                    @endforeach
                </div>
        ...
        <div class="space-y-4">
            ...
            <section class="bg-white rounded-lg border border-slate-200 p-5">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span class="text-xs text-slate-400">vs {{ $anomalies['baseline_months'] }}-mo avg</span>
                </div>
                @if ($anomalies['summary'])
                    <p class="text-sm text-slate-600 mb-3">{{ $anomalies['summary'] }}</p>
                @endif
                    ...
                    <ul class="space-y-2">
                        @foreach ($anomalies['anomalies'] as $a)
                            <li class="flex items-start gap-2 text-sm">
                                <span class="mt-0.5 inline-block w-2 h-2 rounded-full shrink-0 {{ $a['severity'] === 'high' ? 'bg-rose-500' : 'bg-amber-500' }}"></span>
    ...
    <section class="mt-4 bg-white rounded-lg border border-slate-200 p-5">
        <div class="flex items-center justify-between mb-3">
            <div>
                <h2 class="font-semibold">Cash flow forecast · next {{ $forecast['horizon_days'] }} days</h2>
                <p class="text-xs text-slate-500">{{ $forecast['note'] }}</p>
            </div>
            <div class="text-right">
                <div class="text-xs text-slate-500 uppercase tracking-wide">Projected net</div>
                <div class="text-xl font-bold {{ $forecast['projected_net_90d'] >= 0 ? 'text-emerald-600' : 'text-rose-600' }}">{{ $fmt($forecast['projected_net_90d']) }}</div>
            </div>
        </div>
        ...
        @else
            <div class="grid sm:grid-cols-4 gap-3 mb-4 text-sm">
                <div class="rounded-md bg-slate-50 p-3">
                    ...
                    <div class="font-semibold text-emerald-600">{{ $fmt($forecast['daily']['revenue']) }}</div>
                </div>
                <div class="rounded-md bg-slate-50 p-3">
                    <div class="text-xs text-slate-500">Avg daily outflow</div>
                    <div class="font-semibold text-rose-600">{{ $fmt($forecast['daily']['outflow']) }}</div>
                </div>
                <div class="rounded-md bg-slate-50 p-3">
                    <div class="text-xs text-slate-500">Recurring / mo</div>
                    <div class="font-semibold text-slate-700">{{ $fmt($forecast['recurring_monthly']) }}</div>
                </div>
                <div class="rounded-md bg-slate-50 p-3">
                    <div class="text-xs text-slate-500">Avg daily net</div>
                    <div class="font-semibold {{ $forecast['daily']['net'] >= 0 ? 'text-emerald-600' : 'text-rose-600' }}">{{ $fmt($forecast['daily']['net']) }}</div>
                </div>
            </div>
    ...
    <script>
        ...
        (function () {
            ...
            async function ask(question) {
                ...
                try {
                    const res = await fetch(@json(route('insights.chat')), {
                        method: 'POST',
