...
@section('content')
    <div class="max-w-4xl">
        ...
        <form method="POST" action="{{ route('sales.store') }}" class="bg-white rounded-lg border border-slate-200 p-6 space-y-5" x-data="saleForm()">
            @csrf
                ...
                <div>
                    ...
                    <select name="customer_id" class="w-full rounded-md border border-slate-300 px-3 py-2">
                        ...
                        @foreach ($customers as $c)
                            <option value="{{ $c->id }}" {{ old('customer_id') == $c->id ? 'selected' : '' }}>{{ $c->name }}{{ $c->company ? ' ('.$c->company.')' : '' }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                                    ...
                                    <td class="px-3 py-2">
                                        <select :name="`items[${idx}][product_id]`" x-model="item.product_id" @change="pickProduct(idx)" class="w-full rounded border border-slate-300 px-2 py-1 text-sm">
                                            ...
                                            @foreach ($products as $p)
                                                <option value="{{ $p->id }}" data-name="{{ $p->name }}" data-cost="{{ $p->cost_price }}" data-price="{{ $p->sell_price }}">{{ $p->name }}</option>
                                            @endforeach
                                        </select>
            ...
            <div>
                ...
                <textarea name="notes" rows="2" class="w-full rounded-md border border-slate-300 px-3 py-2">{{ old('notes') }}</textarea>
            </div>

            <div class="flex gap-2">
                <button class="px-4 py-2 rounded-md bg-indigo-600 text-white text-sm hover:bg-indigo-700">Record sale</button>
                <a href="{{ route('sales.index') }}" class="px-4 py-2 rounded-md border border-slate-300 bg-white text-sm">Cancel</a>
            </div>
        </form>
    </div>
