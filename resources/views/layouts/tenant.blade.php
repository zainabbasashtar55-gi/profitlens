...
<head>
    ...
    <title>@yield('title', tenant('name')) · ProfitLens</title>
...
@auth
    <div class="flex min-h-screen">
        <aside class="w-60 bg-white border-r border-slate-200 flex flex-col">
            <div class="px-5 py-4 border-b border-slate-200">
                <div class="flex items-center gap-2">
                    ...
                    <div class="leading-tight">
                        ...
                        <div class="text-xs text-slate-500">{{ ucfirst(tenant('plan') ?? 'free') }}</div>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 p-3 text-sm space-y-1">
                ...
                <a href="{{ route('tenant.dashboard') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ $route === 'tenant.dashboard' ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    Dashboard
                </a>
                <a href="{{ route('sales.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'sales.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
                    Sales
                </a>
                <a href="{{ route('invoices.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'invoices.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    Invoices
                </a>
                <a href="{{ route('customers.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'customers.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                ...
                <a href="{{ route('products.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'products.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                ...
                <a href="{{ route('expenses.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'expenses.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                    Expenses
                </a>

                <a href="{{ route('insights.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'insights.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                ...
                <a href="{{ route('team.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'team.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    Team
                </a>
                <a href="{{ route('activity.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'activity.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    Activity log
                </a>
                @if (auth()->user()->hasRole('owner'))
                    <a href="{{ route('billing.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'billing.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                        Billing &amp; plan
                    </a>
                    <a href="{{ route('settings.index') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ str_starts_with($route ?? '', 'settings.') ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                ...
                <a href="{{ route('reports.profit-loss') }}" class="flex items-center gap-2 px-3 py-2 rounded-md {{ $route === 'reports.profit-loss' ? 'bg-indigo-50 text-indigo-700 font-medium' : 'text-slate-700 hover:bg-slate-50' }}">
                ...
                <a href="{{ route('reports.sales.csv') }}" class="flex items-center gap-2 px-3 py-2 rounded-md text-slate-700 hover:bg-slate-50">
                ...
                <a href="{{ route('reports.expenses.csv') }}" class="flex items-center gap-2 px-3 py-2 rounded-md text-slate-700 hover:bg-slate-50">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5m0 0l5-5m-5 5V3"/></svg>
                    Export expenses CSV
                </a>
            </nav>
        ...
        <div class="flex-1 flex flex-col">
            {{-- Top bar: presence avatar cluster + low-cash banner --}}
            <div class="px-8 pt-4 flex items-center justify-end gap-3">
                ...
                <div id="presence-cluster" class="flex items-center -space-x-2">
                    {{-- Filled in by Echo presence subscription --}}
                </div>
            </div>
    ...
    {{-- OCR progress card — pinned bottom-right while OCR is running for any receipt the current user uploaded --}}
